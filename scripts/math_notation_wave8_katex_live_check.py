#!/usr/bin/env python3
"""Live KaTeX DOM check for representative Wave 8 worked examples.
...
Usage (Chromium via Playwright available)::
    ...
    python scripts/math_notation_wave8_katex_live_check.py
"""
...
os.environ.setdefault("SECRET_KEY", "wave8-katex-live-check")
...
OUT = Path("/tmp/wave8_katex_live")
OUT.mkdir(parents=True, exist_ok=True)

PACKAGES = (
    ("CS1-EP001-PKG-2.6-SAMPLING-DISTRIBUTION-STATISTIC", "samp-dist"),
    ("CS1-EP001-PKG-3.3-PERMUTATION-TESTS", "perm"),
    ("CS1-EP001-PKG-2.1-INVERSE-TRANSFORM", "inv-trans"),
    ("CS1-EP001-PKG-CO-5.1-CREDIBLE-INTERVALS", "cred-int"),
    ("CS1-EP001-PKG-CP-3.2-CI-SAMPLE", "ci-sample"),
    ("CS1-EP001-PKG-3.2-BOOTSTRAP-CONFIDENCE-INTERVAL", "boot-ci"),
)
...
def main() -> int:
    ...
    for pack_id, slug in PACKAGES:
        ...
        substance = substance_from_package(
            pack, curriculum_identity="CS1:wave8-live", topic_id=pack.topic_code
        )
        example = next(
            a
...
"""
    ...
    httpd = ThreadingHTTPServer(("127.0.0.1", 8771), Handler)
    ...
    with sync_playwright() as p:
        ...
        for pack_id, slug in PACKAGES:
            url = f"http://127.0.0.1:8771/{slug}.html"
