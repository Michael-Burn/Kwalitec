#!/usr/bin/env python3
"""Live KaTeX DOM check for representative Wave 7 worked examples.
...
Usage (Chromium via Playwright available)::
    ...
    python scripts/math_notation_wave7_katex_live_check.py
"""
...
os.environ.setdefault("SECRET_KEY", "wave7-katex-live-check")
...
OUT = Path("/tmp/wave7_katex_live")
OUT.mkdir(parents=True, exist_ok=True)

PACKAGES = (
    ("CS1-EP001-PKG-CX-4.2-LINK-CANONICAL", "link-canon"),
    ("CS1-EP001-PKG-2.6-F-DISTRIBUTION", "f-dist"),
    ("CS1-EP001-PKG-3.2-CONFIDENCE-INTERVAL-PARAMETER", "ci-param"),
    ("CS1-EP001-PKG-4.2-EXPONENTIAL-FAMILY", "exp-fam"),
    ("CS1-EP001-PKG-4.1-VARIABLE-SELECTION", "var-sel"),
    ("CS1-EP001-PKG-2.2-INDEPENDENCE", "independence"),
)
...
def main() -> int:
    ...
    for pack_id, slug in PACKAGES:
        ...
        substance = substance_from_package(
            pack, curriculum_identity="CS1:wave7-live", topic_id=pack.topic_code
        )
        example = next(
            a
