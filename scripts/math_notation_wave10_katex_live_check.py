#!/usr/bin/env python3
"""Live KaTeX DOM check for representative Wave 10 worked examples.
...
Usage (Chromium via Playwright available)::
    ...
    python scripts/math_notation_wave10_katex_live_check.py
"""
...
os.environ.setdefault("SECRET_KEY", "wave10-katex-live-check")
...
OUT = Path("/tmp/wave10_katex_live")
OUT.mkdir(parents=True, exist_ok=True)

PACKAGES = (
    ("CS1-EP001-PKG-5.1-LOSS-ESTIMATORS", "loss-est"),
    ("CS1-EP001-PKG-5.1-CREDIBLE-INTERVALS", "cred-int"),
    ("CS1-EP001-PKG-REV-BAYESIAN-OMICRON", "bayes-rev"),
    ("CS1-EP001-PKG-REV-HYPOTHESIS-TESTING", "hyp-test"),
    ("CS1-EP001-PKG-REV-MIDSPINE", "midspine"),
    ("CS1-EP001-PKG-4.1-RESPONSE-EXPLANATORY", "resp-expl"),
)
...
def main() -> int:
    ...
    for pack_id, slug in PACKAGES:
        ...
        substance = substance_from_package(
            pack, curriculum_identity="CS1:wave10-live", topic_id=pack.topic_code
        )
        example = next(
            a
...
"""
    ...
    httpd = ThreadingHTTPServer(("127.0.0.1", 8773), Handler)
    ...
    with sync_playwright() as p:
        ...
        for pack_id, slug in PACKAGES:
            url = f"http://127.0.0.1:8773/{slug}.html"
