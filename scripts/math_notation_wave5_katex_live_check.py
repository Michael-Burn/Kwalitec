#!/usr/bin/env python3
"""Live KaTeX DOM check for representative Wave 5 worked examples.
...
Usage (Chromium via Playwright available)::
    ...
    python scripts/math_notation_wave5_katex_live_check.py
"""
...
os.environ.setdefault("SECRET_KEY", "wave5-katex-live-check")
...
OUT = Path("/tmp/wave5_katex_live")
OUT.mkdir(parents=True, exist_ok=True)

PACKAGES = (
    ("CS1-EP001-PKG-2.2-COV-CORR-EXPECTATION", "cov-corr"),
    ("CS1-EP001-PKG-2.3-CONDITIONAL-EXPECTATION", "cond-exp"),
    ("CS1-EP001-PKG-2.2-LINEAR-COMBINATIONS", "lin-comb"),
    ("CS1-EP001-PKG-2.4-MGF-CGF", "mgf-cgf"),
    ("CS1-EP001-PKG-CX-4.2-MEAN-VARIANCE", "mean-var"),
    ("CS1-EP001-PKG-REV-GENERATING-FUNCTIONS", "rev-gf"),
)
...
def main() -> int:
    ...
    for pack_id, slug in PACKAGES:
        ...
        substance = substance_from_package(
            pack, curriculum_identity="CS1:wave5-live", topic_id=pack.topic_code
        )
        example = next(
            a
...
"""
    ...
    httpd = ThreadingHTTPServer(("127.0.0.1", 8769), Handler)
    thread = Thread(target=httpd.serve_forever, daemon=True)
    thread.start()

    from playwright.sync_api import sync_playwright

    # Prefer the installed arm64 headless shell when the Playwright default
    # looks for an x64 path (Rosetta / universal-python mismatch).
    launch_candidates = [
        Path.home()
    ...
    with sync_playwright() as p:
        ...
        for pack_id, slug in PACKAGES:
            url = f"http://127.0.0.1:8769/{slug}.html"
