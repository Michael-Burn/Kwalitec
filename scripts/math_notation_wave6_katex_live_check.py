#!/usr/bin/env python3
"""Live KaTeX DOM check for representative Wave 6 worked examples.
...
Usage (Chromium via Playwright available)::
    ...
    python scripts/math_notation_wave6_katex_live_check.py
"""
...
os.environ.setdefault("SECRET_KEY", "wave6-katex-live-check")
...
OUT = Path("/tmp/wave6_katex_live")
OUT.mkdir(parents=True, exist_ok=True)

PACKAGES = (
    ("CS1-EP001-PKG-CX-4.2-LINEAR-PREDICTOR", "lin-pred"),
    ("CS1-EP001-PKG-2.2-MARGINAL-CONDITIONAL", "marg-cond"),
    ("CS1-EP001-PKG-2.4-MOMENT-VIA-GF", "moment-gf"),
    ("CS1-EP001-PKG-5.1-BAYES-THEOREM", "bayes"),
    ("CS1-EP001-PKG-CX-4.2-EXPONENTIAL-FAMILY", "exp-fam"),
    ("CS1-EP001-PKG-CO-5.1-CREDIBILITY-PREMIUM", "credibility"),
)
...
def main() -> int:
    ...
    for pack_id, slug in PACKAGES:
        ...
        substance = substance_from_package(
            pack, curriculum_identity="CS1:wave6-live", topic_id=pack.topic_code
        )
        example = next(
            a
...
"""
    ...
    with sync_playwright() as p:
        ...
        for pack_id, slug in PACKAGES:
            url = f"http://127.0.0.1:8770/{slug}.html"
