"""In-process health metrics for Daily Plan HTTP cutover (EP-002.7).
...
class DailyPlanCutoverHealthSnapshot:
    """Immutable ops-facing daily-plan cutover health projection."""
...
@dataclass
class DailyPlanCutoverHealthMetrics:
    """Mutable in-process daily-plan cutover aggregator."""
    ...
    def snapshot(self) -> DailyPlanCutoverHealthSnapshot:
        with self._lock:
            ...
            return DailyPlanCutoverHealthSnapshot(
    ...
    def _assess_unlocked(self) -> str:
        ...
        return "ready_for_ep002_8_presentation"


_DEFAULT: DailyPlanCutoverHealthMetrics | None = None


def get_daily_plan_cutover_health_metrics() -> DailyPlanCutoverHealthMetrics:
    global _DEFAULT
    if _DEFAULT is None:
        _DEFAULT = DailyPlanCutoverHealthMetrics()
    return _DEFAULT


def set_daily_plan_cutover_health_metrics(
    metrics: DailyPlanCutoverHealthMetrics | None,
) -> DailyPlanCutoverHealthMetrics | None:
    global _DEFAULT
    previous = _DEFAULT
    _DEFAULT = metrics
    return previous
...
def build_daily_plan_cutover_health_metrics() -> DailyPlanCutoverHealthMetrics:
    return DailyPlanCutoverHealthMetrics()
