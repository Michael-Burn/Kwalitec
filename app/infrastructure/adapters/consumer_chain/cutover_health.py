"""In-process health metrics for Study Insights HTTP cutover (EP-002.5).
...
class StudyInsightsCutoverHealthSnapshot:
    """Immutable ops-facing cutover health projection."""
...
@dataclass
class StudyInsightsCutoverHealthMetrics:
    """Mutable in-process cutover aggregator."""
    ...
    def record(self, event: dict[str, Any]) -> None:
        with self._lock:
            ...
            if not attempted:
                # Ineligible requests still count as legacy fallback traffic
                # for ops awareness, but do not enter eligible denominator.
            ...
            if served:
                ...
                if reason == "twin_exception" or bool(event.get("twin_exception")):
                    # Twin did not succeed; leave twin_success unchanged.
                    pass
            ...
            if status == "aligned":
                ...
                # Already counted via fallback reason when applicable; ensure
                # alignment-tagged limitation events are visible even if reason
    ...
    def _assess_readiness_unlocked(self) -> str:
        ...
        return "ready_for_ep002_6_planning"


_DEFAULT: StudyInsightsCutoverHealthMetrics | None = None


def get_study_insights_cutover_health_metrics() -> StudyInsightsCutoverHealthMetrics:
    global _DEFAULT
    if _DEFAULT is None:
        _DEFAULT = StudyInsightsCutoverHealthMetrics()
    return _DEFAULT


def set_study_insights_cutover_health_metrics(
    metrics: StudyInsightsCutoverHealthMetrics | None,
) -> StudyInsightsCutoverHealthMetrics | None:
    global _DEFAULT
    previous = _DEFAULT
    _DEFAULT = metrics
    return previous
...
def build_study_insights_cutover_health_metrics() -> StudyInsightsCutoverHealthMetrics:
    return StudyInsightsCutoverHealthMetrics()
