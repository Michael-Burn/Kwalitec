"""Mastery state — demonstrated capability beliefs from evidence."""
...
class TopicMasteryRecord:
    """Per-topic mastery belief with explicit confidence."""
    ...
    def create(
        ...
        *,
        ...
        score = _unit_interval(mastery_score, "mastery_score")
...
@dataclass(frozen=True)
class MasteryState:
    """Aggregate mastery beliefs across topics."""

    topic_records: tuple[TopicMasteryRecord, ...] = field(default_factory=tuple)
    overall_score: float = 0.0
    overall_confidence: ConfidenceBand = ConfidenceBand.VERY_LOW
    evidence_ids: tuple[str, ...] = field(default_factory=tuple)

    @classmethod
    def empty(cls) -> MasteryState:
        """Return an empty mastery state."""
        return cls()

    @classmethod
    def create(
        cls,
        topic_records: list[TopicMasteryRecord]
        ...
        *,
        ...
        """Construct MasteryState; overall defaults from topic records."""
        ...
        if overall_confidence is None:
            band = (
                ...
                if not records
                else confidence_band_from_score(
                    sum(_band_rank(r.confidence) for r in records)
                    / (4.0 * len(records))
                )
            )
        else:
    ...
    def record_for(self, topic_id: str) -> TopicMasteryRecord | None:
        """Return the mastery record for ``topic_id``, or None."""
    ...
    def topic_count(self) -> int:
        """Number of topic mastery records."""
