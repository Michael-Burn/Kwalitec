...
| Mastery scoring | Estimated Knowledge (`mastery_score`) from authorised attempts when session evidence write-through is on (`SR_SESSION_SQL_EVIDENCE_COMPANION`); production ON via render.yaml — scored Session practice does not currently update mastery (Stack A EK write retired; companion writes StudyAttempt evidence) |
