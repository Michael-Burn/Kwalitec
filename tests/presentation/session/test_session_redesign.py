...
def test_num_wave1_converted_checkpoints_render_numeric_input(app, package_file: str):
    """NUM Wave 1 conversions: practice stage uses decimal input, not MCQ radios."""
    pkg = _load_package(package_file)
    cp = next(k for k in pkg["knowledge_checks"] if k["kind"] == "checkpoint")
    assert cp["response_type"] == "numeric"
    assert "choices" not in cp
...
def test_num_wave2_converted_checkpoints_render_numeric_input(app, package_file: str):
    """NUM Wave 2 conversions: practice stage uses decimal input, not MCQ radios."""
    pkg = _load_package(package_file)
    cp = next(k for k in pkg["knowledge_checks"] if k["kind"] == "checkpoint")
    assert cp["response_type"] == "numeric"
    assert "choices" not in cp
...
def test_static_asset_version_bumped_for_session_css():
    ...
    assert 'APP_VERSION}-g20"' in version or "-g20" in version
