...
class TestSubjectSupportResolution:
    ...
    def test_coming_soon_cm2_despite_on_disk_syllabus(self):
        """CM2 stays Coming Soon even though a valid syllabus is on disk."""
    ...
    def test_coming_soon_cs2(self):
        info = SubjectSupportService.resolve("IFoA", "CS2")
        assert info.status is SupportStatus.COMING_SOON
        assert info.label == "Coming Soon"
        assert info.allows_plan_creation is False
        assert "coming soon" in info.title.lower()
    ...
    def test_category_summary_ifoa_partial(self):
        ...
        assert "CM2" not in summary.supported_paper_codes
...
class TestWizardSupportSurface:
    ...
    def test_coming_soon_not_selectable(self, logged_in_client):
        ...
        assert 'value="IFoA:CS2"' not in body
    ...
    def test_cm2_coming_soon_cannot_reach_exam_date_despite_syllabus(
        self, logged_in_client
    ):
        """CM2 has a real on-disk syllabus but must not enrol students yet."""
        assert SubjectSupportService.has_curriculum("IFoA", "CM2") is True
        with logged_in_client.session_transaction() as sess:
            sess["wizard_data"] = {
                "exam_category": "IFoA",
...
class TestNoHollowPlans:
    ...
    def test_review_post_refuses_cm2_despite_on_disk_syllabus(self, logged_in_client):
        """Real CM2 syllabus on disk must not create a student study plan yet."""
        assert SubjectSupportService.has_curriculum("IFoA", "CM2") is True
        with logged_in_client.session_transaction() as sess:
            sess["wizard_data"] = {
                "exam_category": "IFoA",
