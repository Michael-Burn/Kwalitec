...
def test_rho_chain_reaches_cr_r1() -> None:
    """RO-015 — Publication Front Wave 0 residual → CR-D1…CR-R1 joint inventory."""
    expected = [
        ("CS1-EP001-PKG-CR-1.1-AIMS-ANALYSIS", "CR-D1"),
        ("CS1-EP001-PKG-CR-1.1-STAGES-TOOLS", "CR-D2"),
        ("CS1-EP001-PKG-CR-1.1-DATA-SOURCES", "CR-D3"),
        ("CS1-EP001-PKG-CR-1.1-REPRODUCIBLE", "CR-D4"),
        ("CS1-EP001-PKG-CR-1.2-EDA-SUMMARIES", "CR-D5"),
        ("CS1-EP001-PKG-CR-1.2-CORRELATION", "CR-D6"),
        ("CS1-EP001-PKG-CR-1.2-PCA", "CR-D7"),
        ("CS1-EP001-PKG-CR-2.1-DISCRETE", "CR-D8"),
        ("CS1-EP001-PKG-CR-2.1-CONTINUOUS", "CR-D9"),
        ("CS1-EP001-PKG-REV-PUBLICATION-FRONT-RHO", "CR-R1"),
    ]
    topic = "1.1"
    last = "CS1-EP001-PKG-REV-SPINE-MEMORY-PI"
    completed: set[str] = {last}
    for package_id, day in expected:
        pack = resolve_active_educational_package(
            subject_id="CS1",
...
def test_pi_revision_hands_off_to_rho() -> None:
    """RO-015 — CP-R1 tomorrow_preview CR-D1 resolves to Rho (not Alpha)."""
    from app.application.educational_packages.loader import EducationalPackageLoader

    pi_ids = {
        "CS1-EP001-PKG-CP-2.1-PROB-QUANTILES",
        "CS1-EP001-PKG-CP-2.2-MARGINAL-CONDITIONAL",
        "CS1-EP001-PKG-CP-2.5-CLT",
        "CS1-EP001-PKG-CP-2.6-RANDOM-SAMPLES",
        "CS1-EP001-PKG-CP-3.1-ESTIMATORS",
        "CS1-EP001-PKG-CP-3.2-CI-SAMPLE",
        "CS1-EP001-PKG-CP-3.3-HYPOTHESIS-TESTING",
        "CS1-EP001-PKG-CP-4.1-LINEAR-REGRESSION",
        "CS1-EP001-PKG-CP-5.1-BAYES-THEOREM",
        "CS1-EP001-PKG-REV-SPINE-MEMORY-PI",
    }
    pack = resolve_active_educational_package(
        subject_id="CS1",
        syllabus_topic_code="1.1",
        completed_package_ids=pi_ids,
        last_completed_package_id="CS1-EP001-PKG-REV-SPINE-MEMORY-PI",
    )
    assert pack is not None
    assert pack.campaign_day == "CR-D1"
    assert pack.package_id == "CS1-EP001-PKG-CR-1.1-AIMS-ANALYSIS"

    # Opening Front cold entry at 1.1 remains Alpha (not Rho Publication Front).
    cold = resolve_active_educational_package(
        subject_id="CS1",
        syllabus_topic_code="1.1",
        completed_package_ids=frozenset(),
    ...
    assert cold is not None
    assert not (cold.campaign_day or "").startswith("CR-")
    assert cold.package_id != "CS1-EP001-PKG-CR-1.1-AIMS-ANALYSIS"

    # Trust Front cold entries unchanged.
    cold51 = resolve_active_educational_package(
        subject_id="CS1",
        syllabus_topic_code="5.1",
        completed_package_ids=frozenset(),
    ...
    # Opening Front cold 2.1 still not Pi/Rho.
    cold21 = resolve_active_educational_package(
        subject_id="CS1",
        syllabus_topic_code="2.1",
        completed_package_ids=frozenset(),
        last_completed_package_id="",
    )
    assert cold21 is not None
    assert not (cold21.campaign_day or "").startswith(("CP-", "CR-"))

    approved = EducationalPackageLoader().all_approved()
    cr = [p for p in approved if (p.campaign_day or "").startswith("CR-")]
    assert len(cr) == 10
    assert len(approved) == 130
