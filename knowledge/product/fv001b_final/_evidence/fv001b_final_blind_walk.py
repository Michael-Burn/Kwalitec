"""FV-001B Final — Founder Studio blind validation (visible UX only).
...
SUBJECT = "CS1F"
TITLE = "CS1F — Actuarial Statistics (FV-001B Final)"
WORKSPACE_ID = f"ws-{SUBJECT.lower()}"

OUT = Path("/tmp/fv001b_final_capture")
SHOTS = OUT / "screenshots"
EV = Path(
    "/Users/kwalitec/Developer/kwalitec/knowledge/product/"
    "fv001b_final/_evidence"
)
...
def main() -> None:
    ...
    with sync_playwright() as p:
        ...
        if note.count():
            note.first.fill(
                "FV-001B Final blind validation — publish verified curriculum"
            )
