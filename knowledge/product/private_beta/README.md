# Private Beta Preparation

**Version:** 1.0  
**Status:** Preparation programme — **no public launch**  
**Related:** Vision 2030, Blueprint, `docs/product/INTERNAL_ALPHA_PLAYBOOK.md`, `.cursor/BETA_READINESS.md`

This folder prepares private beta operations. It does not redesign the product or change educational algorithms.

---

## Purpose

Move from Internal Alpha dogfooding toward a **closed private beta** with disciplined onboarding, feedback, support, and privacy — without public registration or marketing launch.

---

## Workstreams

| Workstream | Document | Status |
|---|---|---|
| Beta onboarding | [`BETA_ONBOARDING.md`](BETA_ONBOARDING.md) | Prepared (process) |
| Feedback system | [`FEEDBACK_SYSTEM.md`](FEEDBACK_SYSTEM.md) | Prepared (process) |
| Issue reporting | [`ISSUE_REPORTING.md`](ISSUE_REPORTING.md) | Prepared (process) |
| Release notes | [`RELEASE_NOTES_POLICY.md`](RELEASE_NOTES_POLICY.md) | Prepared (process) |
| Support workflow | [`SUPPORT_WORKFLOW.md`](SUPPORT_WORKFLOW.md) | Prepared (process) |
| Privacy review | [`PRIVACY_REVIEW.md`](PRIVACY_REVIEW.md) | Prepared (checklist) |

---

## Global constraints

- No public launch.
- No public self-service registration.
- Invite-only accounts (admin / `create-admin` / controlled provisioning).
- EOS canonical runtime; do not reintroduce dual educational brains for beta.
- Align communications with Vision 2030 (learning, not activity; explainable guidance).

---

## Exit toward Version 1

Private beta readiness feeds `knowledge/VERSION_1_READINESS.md` (Beta + Support + Commercial readiness rows).

Educational measurement during beta follows EP-001:

- [`../ep001_product_validation/EDUCATIONAL_VALIDATION_FRAMEWORK.md`](../ep001_product_validation/EDUCATIONAL_VALIDATION_FRAMEWORK.md)
- [`../ep001_product_validation/V1_EXIT_CRITERIA.md`](../ep001_product_validation/V1_EXIT_CRITERIA.md)

Objectives: observe, measure, interview, improve — do not chase feature requests; solve genuine educational problems mapped to outcome IDs O1–O9.
