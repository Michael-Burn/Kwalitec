...
| **ER-TD-H05** | G7 incomplete — no staging/production operator sample / load evidence | P-002.1 G7 | Engineering + Release | Record operator sample; optional HOLD with claim restriction | **HOLD filed** (EI-001.3 — `G7_PERFORMANCE_HOLD.md`); lift when staging sample + load evidence filed |
| **ER-TD-H06** | G8 incomplete — rollback drill + backup acknowledgement for claim class | P-002.1 G8 | Engineering + Release | File drill note + BACKUP_AND_RECOVERY ack | **Closed** (EI-001.3 — `G8_RELIABILITY_EVIDENCE.md`; live restore still recommended before GA marketing) |
| **ER-TD-H07** | G10 incomplete — privacy signatures + dependency critical policy for tag | P-002.1 G10 · V1SP-004 | Security | Close privacy pack for claim class; policy for pip-audit | **Partial** — dependency critical policy Closed (EI-001.2); ops ack EI-001.3; privacy pack residual remains before Stage 1 / V1 |
...
| **ER-TD-H09** | G12 — Version 1 flag matrix not published | P-002.1 G12 · RP-001 FEATURE_FLAG_REGISTER | Product + Release + Engineering | Publish matrix: default ON/OFF, owner, rollback, kill-switch; align `.env.example` / `render.yaml` | **Closed** (EI-001.3 — `VERSION_1_FLAG_MATRIX.md`) |
