...
| D8 | `flask db upgrade` to Alembic head | Head must be `202607300005` |
...
```bash
flask db upgrade
flask db current   # expect 202607300005 (head)
```
...
- Enabling durable flags **without** `flask db upgrade` to `202607300005` will fail writes
