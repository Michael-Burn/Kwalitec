#!/usr/bin/env python3
"""PB-016 — Progressive Confidence evaluation suite (Campaign Pi / LIVE).

Validation only. Does not modify syllabus content, educational packages, Runtime,
Educational Framework, recommendation engine, Student Twin, curriculum, or Wave 15.
Does not begin EP-015.

Builds on RO-014 LIVE harness + PB-015 progressive confidence methodology.
Entry: enrol + seed Continuity Front package history through CO-R1 → Memory Front CP-D1…CP-R1.
"""
...
RO014_BASELINE = Path("/Users/kwalitec/Developer/kwalitec/knowledge/evidence/releases/RO014/results.json")
...
EXPECTED_PACKAGES = {
    "CP-D1": "CS1-EP001-PKG-CP-2.1-PROB-QUANTILES",
    "CP-D2": "CS1-EP001-PKG-CP-2.2-MARGINAL-CONDITIONAL",
    "CP-D3": "CS1-EP001-PKG-CP-2.5-CLT",
    "CP-D4": "CS1-EP001-PKG-CP-2.6-RANDOM-SAMPLES",
    "CP-D5": "CS1-EP001-PKG-CP-3.1-ESTIMATORS",
    "CP-D6": "CS1-EP001-PKG-CP-3.2-CI-SAMPLE",
    "CP-D7": "CS1-EP001-PKG-CP-3.3-HYPOTHESIS-TESTING",
    "CP-D8": "CS1-EP001-PKG-CP-4.1-LINEAR-REGRESSION",
    "CP-D9": "CS1-EP001-PKG-CP-5.1-BAYES-THEOREM",
    "CP-R1": "CS1-EP001-PKG-REV-SPINE-MEMORY-PI",
}
...
ANSWER_TEXT = {
    "novice": "PB-016 beginner: opened the CMP at the named Memory Front hinge, worked slowly through Guided Reading, and answered with the example pattern shown.",
    "mixed": "PB-016 average: followed CMP open/ignore/stop guidance and answered using the session focus.",
    "strong": "PB-016 advanced: applied CMP guidance precisely; connected today's Memory Front hinge to prior Continuity Front substance where relevant.",
    "weak": "PB-016 struggling: re-read the CMP open point, attempted the activity, noted confusion on the stickiest step, and followed stop guidance.",
}
...
def seed_co_r1_history(email: str) -> dict:
    """Seed Continuity Front package history through CO-R1 (ops entry to Memory Front)."""
...
def enrol_persona(c, persona: dict):
    """Baseline enrol for Memory Front entry (start path; history seeded after)."""
...
def detect(day_out: dict) -> str | None:
    ...
    if html_path.is_file():
        try:
            raw_html = html_path.read_text(encoding="utf-8", errors="ignore")[:16000]
            blob += "\n" + raw_html.lower()
        except OSError:
            pass

    m = re.search(r"\b(cp-d[1-9]|cp-r1|co-d[1-9]|co-r1)\b", blob)
    if m:
        return m.group(1).upper()

    for lo, day in HINGE_MAP.items():
        if re.search(rf"syllabus {re.escape(lo)}\b", blob):
            if re.search(r"memory front|pkg-cp-|campaign pi|cp-d", blob):
                return day
            # Opening/Trust Front cold twin — not Memory Front
            if re.search(r"skill\.\s*stop at the stop condition", blob) and not re.search(
                r"memory front|campaign pi|pkg-cp-", blob
            ):
                continue
            if re.search(r"campaign pi|memory front|pkg-cp-|spine re-audit", blob):
                return day
    ...
    if re.search(
        ...
        return "CP-R1"
    if re.search(r"campaign omicron revision|rev-bayesian-omicron", blob):
        return "CO-R1"
    return None
    ...
    for i in range(14):
        ...
        if f"/session/{sid}/activity/answer" in html:
            ...
            data = {
                ...
                "session_id": sid,
                "activity_id": aid,
                "response": f"PB-016 continue {expected_day}: applied CMP guidance.",
            }
    ...
    if reflection_ok and "csrf_token" in html and "Not Found" not in c.title(html):
        _, final, html = c.post(
            f"/session/{sid}/reflection/continue",
            {
                "csrf_token": c.csrf(html),
                "session_id": sid,
                "reflection_note": f"PB-016 continue {expected_day}: noted stickiest CMP cue.",
                "submit": "Continue",
            },
        )
    ...
    if "completion_status" in html:
        _, final, html = c.post(
            f"/session/{sid}/finish",
            {
                ...
                "notes": f"PB-016 continue {expected_day}: finished planned study.",
        ...
        def post_hook(path, data=None, **kw):
            ...
            if isinstance(path, str) and path.endswith("/reflection/continue"):
                data["reflection_note"] = (
                    f"PB-016 {persona['slug']} {expected_day}: completed; noted stickiest CMP cue."
                )
...
def honest_pi_stop(body: str) -> bool:
    return any(
        n in body
        for n in (
            "honest stop",
...
def score_cp(day_out: dict, actual: str) -> dict:
    ...
    if is_rev and dims["tomorrow_confidence"] == "FAIL" and finished:
        ...
        residuals.append("RO14-R1")
    if not chrome and not is_rev and finished:
        # Learning-day chrome miss: numeric FAIL on tomorrow_confidence (8/9); progressive PASS still if score>=8
        day_out["chrome_residual"] = True
        residuals.append("RO14-R1")
    if is_rev and not q6:
        day_out["revision_q6_residual"] = True
        residuals.append("RO14-R3")
...
def classify_defects(day_out: dict, actual: str | None) -> list[dict]:
    ...
    if audit.get("is_fallback_shell"):
        defects.append(
            {
                "severity": "Critical",
                "id": "PB16-CRIT-FALLBACK",
                "finding": f"Fallback shell on sitting detected as {actual}",
                "ef_class": "PI",
            }
        )
    if ("until-exam trust" in body or "100% cs1" in body) and not honest_pi_stop(body):
        defects.append(
            {
                "severity": "Critical",
                "id": "PB16-CRIT-LEAK",
                "finding": "Until-exam / 100% CS1 claim appeared without honest stop during Pi walk",
                "ef_class": "EC",
            }
        )
    if actual and actual.startswith("CP-") and not day_out.get("finished"):
        defects.append(
            {
                "severity": "Critical",
                "id": "PB16-CRIT-UNFINISHED",
                "finding": f"Certified Pi day {actual} did not finish",
                "ef_class": "PI",
            }
        )
    if actual and actual.startswith("CP-") and day_out.get("verdict") == "FAIL":
        defects.append(
            {
                "severity": "Major",
                "id": "PB16-MAJOR-SCORE",
    ...
    if day_out.get("chrome_residual") and actual and actual.startswith("CP-"):
        defects.append(
            {
                "severity": "Minor",
                "id": "PB16-MINOR-CHROME",
                "finding": f"Tomorrow chrome residual on {actual} (RO14-R1 class)",
                "ef_class": "PI",
                "residual": "RO14-R1",
            }
        )
        ...
        defects.append(
            {
                "severity": "Minor",
                "id": "PB16-MINOR-Q6",
                "finding": f"Revision Q6 Learning-oriented residual on {actual} (RO14-R3 class)",
                "ef_class": "PI",
                "residual": "RO14-R3",
            }
        )
    if actual and not actual.startswith("CP-") and actual:
        defects.append(
            {
                "severity": "Minor",
                "id": "PB16-MINOR-LABEL",
                "finding": (
                    f"Ops label desync sitting observed as {actual} "
                    "before/during Memory Front entry (RO14-R2 class)"
                ),
                "ef_class": "PI",
                "residual": "RO14-R2",
            }
...
def save_runtime_checkpoint(
    ...
    ck = {
        ...
        "remaining_cp_days": [d for d in TARGET if d not in {t["day"] for t in trajectory}],
...
def run_persona(persona: dict) -> dict:
    ...
    email = f"pb016.cp.{slug}.{int(time.time())}@example.com"
    ...
    st = create_user(email, f"PB016 {persona['label']}", password)
    ...
    for _n in range(40):
        ...
        try:
            ...
            expect = next((d for d in TARGET if d not in seen), "CP-R1")
            ...
            if actual and actual.startswith("CP-"):
                day_out = score_cp(day_out, actual)
                ...
                if actual not in seen and day_out.get("verdict") == "PASS":
                    ...
                    print(
                        f"  cp FAIL-ish {actual} score={day_out.get('score_over_9')} "
            ...
            for hp in html_dir.glob(f"day{idx}_*.html"):
                if actual and actual.startswith("CP-") and hp.name.endswith("_reading.html"):
                    continue
                try:
                    hp.unlink()
                except OSError:
...
def regression_vs_pi(persona_results: list[dict]) -> dict:
    baseline = json.loads(RO014_BASELINE.read_text()) if RO014_BASELINE.exists() else {}
    expected_days = set(TARGET)
    expected_packages = dict(EXPECTED_PACKAGES)
    known_residuals = {"RO14-R1", "RO14-R2", "RO14-R3", "RO14-R4"}
    regressions = []
    package_mismatches = []
    new_defects = []
    ...
    return {
        ...
        "order_consistent_with_cp_d1_to_cp_r1": order_consistent,
...
def aggregate(persona_results: list[dict]) -> dict:
    ...
    reg = regression_vs_pi(persona_results)
    prog_keys = [
        "recommendation_consistency",
        "weak_area_identification",
        "mission_sequencing",
    ...
    out = {
        ...
        "regression_vs_ro014": reg,
        "coverage_held": "63/72 (87.5%)",
