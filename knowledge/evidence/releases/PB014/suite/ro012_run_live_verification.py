#!/usr/bin/env python3
"""RO-012 — Wave 1 LIVE verification (browserless).
...
CERT = ROOT / "certified"
EVID_REPO = Path(
    "/Users/kwalitec/Developer/kwalitec/knowledge/evidence/releases/RO012"
)
EXPECTED_COMMIT = "a800c85f602b68d1380ae355c0d2839403018995"
...
def complete_session(c: mod.Client, day_idx: int, expected_day: str) -> dict:
    ...
    for i in range(12):
        ...
        if f"/session/{sid}/activity/answer" in html:
            ...
            data = {
                ...
                "activity_id": aid,
                "response": (
                    f"RO-012 day {expected_day}: applied CMP guidance and answered as directed."
                ),
            }
    ...
    if reflection_ok and "csrf_token" in html and "Not Found" not in c.title(html):
        _, final, html = c.post(
            f"/session/{sid}/reflection/continue",
            {
                ...
                "session_id": sid,
                "reflection_note": (
                    f"RO-012 {expected_day}: completed activities; noted stickiest CMP cue."
                ),
                "submit": "Continue",
            },
        )
    ...
    if "completion_status" in html:
        _, final, html = c.post(
            f"/session/{sid}/finish",
            {
                ...
                "notes": f"RO-012 {expected_day}: finished planned study.",
    ...
    # Package-path PASS: finished + reflection + CMP + no fallback (RO-002…RO-012 policy)
