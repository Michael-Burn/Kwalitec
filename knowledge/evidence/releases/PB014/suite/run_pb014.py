#!/usr/bin/env python3
"""PB-014 — Progressive Confidence evaluation suite (Campaign Xi / LIVE).

Validation only. Does not modify syllabus content, educational packages, or Runtime.
Does not begin Wave 13 / EP-013.
...
ROOT = Path("/tmp/pb014")
RO012 = Path("/tmp/ro012")
spec = importlib.util.spec_from_file_location("ro012", str(RO012 / "run_live_verification.py"))
...
RO012_BASELINE = Path("/Users/kwalitec/Developer/kwalitec/knowledge/evidence/releases/RO012/results.json")
...
ANSWER_TEXT = {
    "novice": "PB-014 beginner: opened the CMP at the named section, worked slowly through the Guided Reading hinges, and answered with the example pattern shown.",
    "mixed": "PB-014 average: followed CMP open/ignore/stop guidance and answered using the session focus.",
    "strong": "PB-014 advanced: applied CMP guidance precisely; connected today's GLM hinge to prior Continuity Front linear-regression ideas where relevant.",
    "weak": "PB-014 struggling: re-read the CMP open point, attempted the activity, noted confusion on the stickiest step, and followed stop guidance.",
}
...
EXPECTED_PACKAGES = {
    "CX-D1": "CS1-EP001-PKG-CX-4.2-EXPONENTIAL-FAMILY",
    "CX-D2": "CS1-EP001-PKG-CX-4.2-MEAN-VARIANCE",
    "CX-D3": "CS1-EP001-PKG-CX-4.2-LINK-CANONICAL",
    "CX-D4": "CS1-EP001-PKG-CX-4.2-FACTORS-INTERACTIONS",
    "CX-D5": "CS1-EP001-PKG-CX-4.2-LINEAR-PREDICTOR",
    "CX-D6": "CS1-EP001-PKG-CX-4.2-DEVIANCE-ESTIMATION",
    "CX-D7": "CS1-EP001-PKG-CX-4.2-MODEL-CHOICE",
    "CX-D8": "CS1-EP001-PKG-CX-4.2-RESIDUALS",
    "CX-D9": "CS1-EP001-PKG-CX-4.2-GOODNESS-TESTS",
    "CX-D10": "CS1-EP001-PKG-CX-4.2-FIT-INTERPRET",
    "CX-R1": "CS1-EP001-PKG-REV-GLM-XI",
}
...
def create_user(email: str, name: str, password: str) -> dict:
    ...
    payload_path = ACCT / f"tmp_pb014_{email.split('@')[0].replace('.', '_')}.json"
...
def detect(day_out: dict) -> str | None:
    ...
    if html_path.exists():
        try:
            body += "\n" + html_path.read_text()[:8000].lower()
        except OSError:
            pass

    # Explicit campaign day markers (prefer over LO when present)
    m = re.search(r"\b(cx-d10|cx-d[1-9]|cx-r1|cn-d[1-5]|cn-r1|cm-d[1-5]|cm-r1|cl-d[1-8]|cl-r1|ck-d[1-6]|ck-r1)\b", body)
    if m:
        return m.group(1).upper().replace("CX-D10", "CX-D10")

    # Prefer purpose/open CMP lead-line over "stop before later LO" mentions
    lead = re.search(
        r"(?:purpose of this reading:[^\n]{0,160}|open:\s*cmp[^\n]{0,120})"
        r".*?syllabus (4\.2\.10|4\.2\.[1-9]|4\.1\.[1-5]|3\.3\.[1-5]|3\.2\.[1-8]|3\.1\.[1-6])\b",
        body,
        re.S,
    )
    if lead:
    ...
    # Syllabus lead-lines — 4.2.10 before 4.2.1–9
        ...
        def post_hook(path, data=None, **kw):
            ...
            if isinstance(path, str) and path.endswith("/reflection/continue"):
                data["reflection_note"] = (
                    f"PB-014 {persona['slug']} {expected_day}: completed; noted stickiest CMP cue."
                )
...
def honest_xi_stop(body: str) -> bool:
    return any(
        n in body
        for n in (
            "honest stop",
            ...
            "first-pass spine",
            "campaign xi / cs1-014",
            "campaign xi / volume cs1-014",
            "wave 0 honesty",
            "trust front",
            "5.1",
            "not absorb",
            "do not begin syllabus 5",
            "don't begin syllabus 5",
            "successor",
            "spine re-audit",
        )
    )
...
def score_xi(day_out: dict, actual: str) -> dict:
    """Nine-dimension educational confidence scoring for LIVE-certified Xi days."""
...
def programme_metrics_for_day(day_out: dict, actual: str, prior_days: list[str]) -> dict:
    ...
    # Continuity: honest Xi stop / no premature until-exam / 5.1 trophy leak.
    if "syllabus 5.1" in body or "chapter 5" in body or "until-exam" in body:
        no_leak = honest_xi_stop(body)
    else:
        no_leak = True
...
def classify_defects(day_out: dict, actual: str | None) -> list[dict]:
    ...
    if audit.get("is_fallback_shell"):
        defects.append(
            {
                "severity": "Critical",
                "id": "PB14-CRIT-FALLBACK",
                "finding": f"Fallback shell on sitting detected as {actual}",
                "ef_class": "PI",
            }
        )
    if ("syllabus 5.1" in body or "until-exam trust" in body) and not honest_xi_stop(body):
        defects.append(
            {
                "severity": "Critical",
                "id": "PB14-CRIT-LEAK",
                "finding": "Unpublished / trophy geography appeared without honest stop during Xi walk",
                "ef_class": "EC",
            }
        )
    if actual and actual.startswith("CX-") and not day_out.get("finished"):
        defects.append(
            {
                "severity": "Critical",
                "id": "PB14-CRIT-UNFINISHED",
                "finding": f"Certified Xi day {actual} did not finish",
                "ef_class": "PI",
            }
        )
    if actual and actual.startswith("CX-") and day_out.get("verdict") == "FAIL":
        defects.append(
            {
                "severity": "Major",
                "id": "PB14-MAJOR-SCORE",
    ...
    if day_out.get("chrome_residual") and actual and actual.startswith("CX-"):
        defects.append(
            {
                "severity": "Minor",
                "id": "PB14-MINOR-CHROME",
                "finding": f"Tomorrow chrome residual on {actual} (RO12-R3 class)",
                "ef_class": "PI",
                "residual": "RO12-R3",
            }
        )
        ...
        defects.append(
            {
                "severity": "Minor",
                "id": "PB14-MINOR-Q6",
                "finding": f"Revision Q6 Learning-oriented residual on {actual} (RO12-R2 class)",
                "ef_class": "PI",
                "residual": "RO12-R2",
            }
        )
    if actual and not actual.startswith("CX-") and actual:
        defects.append(
            {
                "severity": "Minor",
                "id": "PB14-MINOR-LABEL",
                "finding": (
                    ...
                    "before/during Xi entry (RO12-R1 class)"
                ),
                "ef_class": "PI",
                "residual": "RO12-R1",
            }
...
def pick_expect(seen: dict, last_detected: str | None) -> str:
    """Bind expect-day for chrome once Continuity Front reaches Xi entry."""
    entered_xi = bool(seen) or (last_detected in TARGET) or last_detected == "CN-R1"
    if entered_xi:
        return next((d for d in TARGET if d not in seen), "CX-R1")
    ...
    return "CK-D1" if "CK-D1" in ro.SNIPPETS else "CX-D1"


def run_persona(persona: dict) -> dict:
    slug = persona["slug"]
    password = PASS_FILE.read_text().strip()
    email = f"pb014.xi.{slug}.{int(time.time())}@example.com"
    print(f"\n=== persona {slug} provision {email}", flush=True)
    st = create_user(email, f"PB014 {persona['label']}", password)
    ...
    for _n in range(120):
        ...
        try:
            ...
            if actual and actual.startswith("CX-"):
                day_out = score_xi(day_out, actual)
                ...
                if actual not in seen and day_out.get("verdict") == "PASS":
                    ...
                    print(
                        f"  xi FAIL-ish {actual} score={day_out.get('score_over_9')} "
                ...
                if actual:
                    all_defects.extend(
                        d for d in defects if d.get("id") == "PB14-MINOR-LABEL"
                    )
                print(
            ...
            for hp in html_dir.glob(f"day{idx}_*.html"):
                if actual and actual.startswith("CX-") and hp.name.endswith("_reading.html"):
                    continue
                try:
                    hp.unlink()
                except OSError:
...
def regression_vs_xi(persona_results: list[dict]) -> dict:
    baseline = json.loads(RO012_BASELINE.read_text()) if RO012_BASELINE.exists() else {}
    expected_days = set(TARGET)
    expected_packages = dict(EXPECTED_PACKAGES)
    known_residuals = {"RO12-R1", "RO12-R2", "RO12-R3"}
    regressions = []
    package_mismatches = []
    new_defects = []
    ...
    return {
        ...
        "baseline_xi_days": sorted(expected_days),
...
def build_results(results: list[dict], health: dict) -> dict:
    ...
    regression = regression_vs_xi(results)
    ...
    return {
        "programme": "PB-014 Progressive Educational Confidence (Xi)",
        "host": mod.BASE,
        ...
        "regression_vs_campaign_xi": regression,
        "verdict": overall,
    }
...
def main() -> int:
    ...
    (EVID_REPO / "suite" / "run_pb014.py").write_text(suite_src.read_text())
