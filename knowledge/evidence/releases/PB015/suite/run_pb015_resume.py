#!/usr/bin/env python3
"""PB-015 resilient resume — continue an existing Internal Alpha persona from checkpoint.
...
def persist_certified_sitting(ck: dict, slug: str, day_out: dict, actual: str, idx: int) -> None:
    traj_item = {
        ...
        "curriculum_topic": "5.1" if actual.startswith("CO-D") else "5.1-revision",
        "campaign_day": actual,
    ...
    ck["remaining_co_days"] = [d for d in m.TARGET if d not in set(ck["certified_days_complete"])]
    ck["status"] = "complete" if not ck["remaining_co_days"] else "in_progress"
    save_ckpt(ck)
...
def resume_persona(slug: str) -> dict:
    ...
    print(f"\n=== RESUME {slug} {email} certified={ck.get('certified_days_complete')} remaining={ck.get('remaining_co_days')}", flush=True)
    op_note(ck, "resume_start", remaining=ck.get("remaining_co_days"))
    save_ckpt(ck)
    ...
    for _n in range(80):
        ...
        try:
            ...
            if actual and actual.startswith("CO-"):
                ...
                day_out = m.score_co(day_out, actual)
                ...
                defects = [
                    ...
                    or d.get("id") in ("PB15-MINOR-CHROME", "PB15-MINOR-Q6")
                ]
                ...
                if day_out.get("verdict") == "PASS":
                    ...
                    print(
                        f"  co educational FAIL-ish {actual} score={day_out.get('score_over_9')} "
...
def main() -> int:
    if len(sys.argv) < 2:
        print("Usage: run_pb015_resume.py <slug>")
        return 2
    slug = sys.argv[1]
