#!/usr/bin/env python3
"""PB-012 — Progressive Confidence evaluation suite (Campaign Mu / LIVE).

Validation only. Does not modify syllabus content, educational packages, or Runtime.
Does not begin Wave 11 / EP-011.

Builds on RO-010 LIVE harness + PB-011 progressive confidence methodology.
"""
...
ROOT = Path("/tmp/pb012")
RO010 = Path("/tmp/ro010")
spec = importlib.util.spec_from_file_location("ro010", str(RO010 / "run_live_verification.py"))
...
RO010_BASELINE = Path("/Users/kwalitec/Developer/kwalitec/knowledge/evidence/releases/RO010/results.json")
...
CODE_TO_DAY = {f"3.3.{i}": f"CM-D{i}" for i in range(1, 6)}
...
ANSWER_TEXT = {
    "novice": "PB-012 beginner: opened the CMP at the named section, worked slowly through the Guided Reading hinges, and answered with the example pattern shown.",
    "mixed": "PB-012 average: followed CMP open/ignore/stop guidance and answered using the session focus.",
    "strong": "PB-012 advanced: applied CMP guidance precisely; connected today's hypothesis-testing hinge to prior Continuity Front interval ideas where relevant.",
    "weak": "PB-012 struggling: re-read the CMP open point, attempted the activity, noted confusion on the stickiest step, and followed stop guidance.",
}
...
EXPECTED_PACKAGES = {
    "CM-D1": "CS1-EP001-PKG-3.3-HYPOTHESIS-CONCEPTS",
    "CM-D2": "CS1-EP001-PKG-3.3-BASIC-TESTS",
    "CM-D3": "CS1-EP001-PKG-3.3-PERMUTATION-TESTS",
    "CM-D4": "CS1-EP001-PKG-3.3-CHI-SQUARE-GOF",
    "CM-D5": "CS1-EP001-PKG-3.3-CONTINGENCY-INDEPENDENCE",
    "CM-R1": "CS1-EP001-PKG-REV-HYPOTHESIS-TESTING",
}
...
def create_user(email: str, name: str, password: str) -> dict:
    ...
    payload_path = ACCT / f"tmp_pb012_{email.split('@')[0].replace('.', '_')}.json"
...
def detect(day_out: dict) -> str | None:
    ...
    if m:
        return CODE_TO_DAY[f"3.3.{m.group(1)}"]
        ...
        def post_hook(path, data=None, **kw):
            ...
            if isinstance(path, str) and path.endswith("/reflection/continue"):
                data["reflection_note"] = (
                    f"PB-012 {persona['slug']} {expected_day}: completed; noted stickiest CMP cue."
                )
...
def honest_mu_stop(body: str) -> bool:
    return any(
        n in body
        for n in (
            ...
            "remainder-spine",
            "remainder / spine",
            "do not claim chapter 3",
            "not until-exam",
            "until-exam trust",
            "not first-pass spine",
            "3.3 pilot arc closed",
            "campaign mu / cs1-012 complete after",
            "do not begin syllabus 3.4",
            "don't begin syllabus 3.4",
            "not begin syllabus 3.4",
        )
    )


def score_mu(day_out: dict, actual: str) -> dict:
    """Nine-dimension educational confidence scoring for LIVE-certified Mu days."""
...
def programme_metrics_for_day(day_out: dict, actual: str, prior_days: list[str]) -> dict:
    ...
    # Continuity: honest Mu stop / no premature unpublished first-pass leak.
    if "syllabus 3.4" in body or "chapter 3 complete" in body:
        no_leak = honest_mu_stop(body)
    else:
        no_leak = True
...
def classify_defects(day_out: dict, actual: str | None) -> list[dict]:
    ...
    if audit.get("is_fallback_shell"):
        defects.append(
            {
                "severity": "Critical",
                "id": "PB12-CRIT-FALLBACK",
                "finding": f"Fallback shell on sitting detected as {actual}",
                "ef_class": "PI",
            }
        )
    if ("syllabus 3.4" in body or "chapter 3 complete" in body) and not honest_mu_stop(body):
        defects.append(
            {
                "severity": "Critical",
                "id": "PB12-CRIT-LEAK",
                "finding": "Unpublished / trophy geography appeared without honest stop during Mu walk",
                "ef_class": "EC",
            }
        )
    if actual and actual.startswith("CM-") and not day_out.get("finished"):
        defects.append(
            {
                "severity": "Critical",
                "id": "PB12-CRIT-UNFINISHED",
                "finding": f"Certified Mu day {actual} did not finish",
                "ef_class": "PI",
            }
        )
    if actual and actual.startswith("CM-") and day_out.get("verdict") == "FAIL":
        defects.append(
            {
                "severity": "Major",
                "id": "PB12-MAJOR-SCORE",
    ...
    if day_out.get("chrome_residual") and actual and actual.startswith("CM-"):
        defects.append(
            {
                "severity": "Minor",
                "id": "PB12-MINOR-CHROME",
                "finding": f"Tomorrow chrome residual on {actual} (RO10-R3 class)",
                "ef_class": "PI",
                "residual": "RO10-R3",
            }
        )
        ...
        defects.append(
            {
                "severity": "Minor",
                "id": "PB12-MINOR-Q6",
                "finding": f"Revision Q6 Learning-oriented residual on {actual} (RO10-R2 class)",
                "ef_class": "PI",
                "residual": "RO10-R2",
            }
        )
    if actual and not actual.startswith("CM-") and actual:
        defects.append(
            {
                "severity": "Minor",
                "id": "PB12-MINOR-LABEL",
                "finding": (
                    ...
                    "before/during Mu entry (RO10-R1 class)"
                ),
                "ef_class": "PI",
                "residual": "RO10-R1",
            }
...
def pick_expect(seen: dict, last_detected: str | None) -> str:
    """Bind expect-day for chrome once Continuity Front reaches Mu entry."""
    entered_mu = bool(seen) or (last_detected in TARGET) or last_detected == "CL-R1"
    if entered_mu:
        return next((d for d in TARGET if d not in seen), "CM-R1")
    if last_detected and last_detected.startswith("CL-"):
        # Still on Lambda transit — bind next Lambda day if known, else CL-D1
    ...
    return "CK-D1" if "CK-D1" in ro.SNIPPETS else "CM-D1"


def run_persona(persona: dict) -> dict:
    slug = persona["slug"]
    password = PASS_FILE.read_text().strip()
    email = f"pb012.mu.{slug}.{int(time.time())}@example.com"
    print(f"\n=== persona {slug} provision {email}", flush=True)
    st = create_user(email, f"PB012 {persona['label']}", password)
    ...
    for _n in range(70):
        if all(k in seen for k in TARGET):
            break
        print(f"  day {idx} mu_seen={sorted(seen)} last={last_detected}", flush=True)
        try:
            ...
            if actual and actual.startswith("CM-"):
                day_out = score_mu(day_out, actual)
                ...
                if actual not in seen and day_out.get("verdict") == "PASS":
                    ...
                    print(
                        f"  mu FAIL-ish {actual} score={day_out.get('score_over_9')} "
                ...
                if actual:
                    all_defects.extend(
                        d for d in defects if d.get("id") == "PB12-MINOR-LABEL"
                    )
                print(
...
def regression_vs_mu(persona_results: list[dict]) -> dict:
    baseline = json.loads(RO010_BASELINE.read_text()) if RO010_BASELINE.exists() else {}
    expected_days = set(TARGET)
    expected_packages = dict(EXPECTED_PACKAGES)
    known_residuals = {"RO10-R1", "RO10-R2", "RO10-R3"}
    regressions = []
    package_mismatches = []
    new_defects = []
    ...
    return {
        ...
        "baseline_mu_days": sorted(expected_days),
...
def main() -> int:
    ...
    (EVID_REPO / "suite" / "run_pb012.py").write_text(suite_src.read_text())

    probe = mod.Client("pb012_probe")
    ...
    regression = regression_vs_mu(results)
