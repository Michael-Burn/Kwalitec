#!/usr/bin/env python3
"""PB-010 — Progressive Confidence evaluation suite (Campaign Kappa / LIVE).

Validation only. Does not modify syllabus content, educational packages, or Runtime.
Does not begin Wave 9.

Builds on RO-008 LIVE harness + PB-009 progressive confidence methodology.
"""
...
ROOT = Path("/tmp/pb010")
RO008 = Path("/tmp/ro008")
spec = importlib.util.spec_from_file_location("ro008", str(RO008 / "run_live_verification.py"))
...
EVID_REPO = Path("/Users/kwalitec/Developer/kwalitec/knowledge/evidence/releases/PB010")
PASS_FILE = ACCT / "shared_pass.txt"
RO008_BASELINE = Path("/Users/kwalitec/Developer/kwalitec/knowledge/evidence/releases/RO008/results.json")

TARGET = ["CK-D1", "CK-D2", "CK-D3", "CK-D4", "CK-D5", "CK-D6", "CK-R1"]
CODE_TO_DAY = {f"3.1.{i}": f"CK-D{i}" for i in range(1, 7)}

# Persona definitions — diverse Internal Alpha cohort
PERSONAS = [
    {
        "slug": "beginner",
...
ANSWER_TEXT = {
    "novice": "PB-010 beginner: opened the CMP at the named section, worked slowly through the Guided Reading hinges, and answered with the example pattern shown.",
    "mixed": "PB-010 average: followed CMP open/ignore/stop guidance and answered using the session focus.",
    "strong": "PB-010 advanced: applied CMP guidance precisely; connected today's estimator hinge to prior Continuity Front sampling ideas where relevant.",
    "weak": "PB-010 struggling: re-read the CMP open point, attempted the activity, noted confusion on the stickiest step, and followed stop guidance.",
}
...
def create_user(email: str, name: str, password: str) -> dict:
    ...
    payload_path = ACCT / f"tmp_pb010_{email.split('@')[0].replace('.', '_')}.json"
            ...
            if n == "experience":
                ...
                if pref in vs:
                    ...
                    data[n] = next(
                        ...
                        next((v for v in vs if v != "first_sitting" and "first" not in v.lower()), vs[-1]),
                    )
                else:
                    data[n] = (
                        "first_sitting"
...
def detect(day_out: dict) -> str | None:
    ...
    # Prefer syllabus code for Kappa Learning days
    m = re.search(r"syllabus 3\.1\.([1-6])", body)
    if m:
        return CODE_TO_DAY[f"3.1.{m.group(1)}"]
...
def patch_session_answers(persona: dict):
    ...
    def wrapped(c, day_idx: int, expected_day: str):
        # Temporarily patch Client.post to rewrite activity answers
        orig_post = c.post

        def post_hook(path, data=None, **kw):
            data = dict(data or {})
            if path.endswith("/activity/answer"):
                data["response"] = ANSWER_TEXT.get(
                    persona.get("answer_style", "mixed"), ANSWER_TEXT["mixed"]
                )
                if "confidence" in data or True:
                    # only set if field likely present — complete_session adds when in fields
                    pass
                # complete_session sets confidence before post; rewrite if present
                if "confidence" in data:
                    data["confidence"] = persona.get("confidence", "3")
                if "confidence_level" in data:
                    data["confidence_level"] = persona.get("confidence_level", "medium")
            if path.endswith("/reflection/continue"):
                data["reflection_note"] = (
                    f"PB-010 {persona['slug']} {expected_day}: completed; noted stickiest CMP cue."
                )
            return orig_post(path, data, **kw)

        c.post = post_hook
        try:
            # Also rewrite confidence fields that complete_session builds before post
            # by patching the function body via wrapping the answer-data construction:
            out = orig(c, day_idx, expected_day)
        finally:
            c.post = orig_post
        return out

    # Deeper patch: rewrite complete_session inline confidence assignment via source-level hook
    def complete_persona(c, day_idx: int, expected_day: str):
        out = {
            "day_index": day_idx,
            "expected_day": expected_day,
            "expected_package_id": ro.SNIPPETS[expected_day]["package_id"],
            "finished": False,
            "reading": None,
            "checklist": {},
            "tomorrow_preview": {},
        ...
        # Call original then re-run is heavy; instead call orig with post hook that rewrites
        # confidence keys that complete_session puts in data dict before post.
        import types

        real_complete = orig

        # Patch re.search path for confidence by wrapping Client.post AND mutating
        # the data dict that complete_session builds — achieved by intercepting post.
        orig_post = c.post
        ...
        c.post = post_hook
        try:
            out = real_complete(c, day_idx, expected_day)
...
def score_kappa(day_out: dict, actual: str) -> dict:
    """Nine-dimension educational confidence scoring (PB-004…PB-009 law) for Kappa."""
    ...
    # Always bind package_id to detected Kappa day (complete_session scaffolding may use CK-D1).
...
def programme_metrics_for_day(day_out: dict, actual: str, prior_days: list[str]) -> dict:
    """Six PB-010 programme measurement dimensions (observational)."""
    checklist = day_out.get("checklist") or {}
    audit = (day_out.get("reading") or {}).get("audit") or {}
    finished = bool(day_out.get("finished"))
    not_fallback = not bool(audit.get("is_fallback_shell"))
    ...
    # Mission sequencing: day follows expected Continuity Front order
    expected_next = None
    if prior_days:
        last = prior_days[-1]
        try:
    ...
    # Continuity: no premature 3.2 / unpublished geography.
    # Honest Kappa Revision stop copy says "do not begin syllabus 3.2" / "not Chapter 3 complete"
    # — those are anti-leak guarantees, not leaks.
    if "syllabus 3.2" in body:
        honest_stop = any(
            n in body
            for n in (
                "do not begin syllabus 3.2",
                "don't begin syllabus 3.2",
                "not begin syllabus 3.2",
                "do not begin syllabus 3.2 in this",
                "await 3.2",
    ...
    # final syllabus_continuity uses no_leak directly below

    # Weak-area identification: revision targets estimator hinges / retrieval
    weak_ok = True
    if is_rev:
        weak_ok = any(
            n in body
    ...
    # Recommendation consistency placeholder at day level (filled at cohort)
    rec_ok = actual in TARGET and not_fallback and finished

    # Confidence calibration: finished + trajectory-compatible score; chrome tracked
    calib_ok = finished and day_out.get("confidence_level") in ("HIGH", "MEDIUM")

    # Explanation usefulness: purpose/focus or CMP framing present
...
def classify_defects(day_out: dict, actual: str | None) -> list[dict]:
    ...
    if "syllabus 3.2" in body:
        ...
        if not honest_stop:
            defects.append(
                {
                    "severity": "Critical",
                    "id": "PB10-CRIT-LEAK-32",
                    "finding": "Unpublished 3.2 geography appeared during Kappa walk",
                    "ef_class": "EC",
                }
            )
    if actual and actual.startswith("CK-") and not day_out.get("finished"):
        defects.append(
            {
                "severity": "Critical",
                "id": "PB10-CRIT-UNFINISHED",
                "finding": f"Certified Kappa day {actual} did not finish",
                "ef_class": "PI",
            }
        )
    if actual and actual.startswith("CK-") and day_out.get("verdict") == "FAIL":
        defects.append(
            {
                "severity": "Major",
                "id": "PB10-MAJOR-SCORE",
    ...
    if day_out.get("chrome_residual"):
        defects.append(
            {
                "severity": "Minor",
                "id": "PB10-MINOR-CHROME",
                "finding": f"Tomorrow chrome residual on {actual} (RO8-R3 class)",
                "ef_class": "PI",
                "residual": "RO8-R3",
            }
        )
        ...
        defects.append(
            {
                "severity": "Minor",
                "id": "PB10-MINOR-Q6",
                "finding": f"Revision Q6 Learning-oriented residual on {actual} (RO8-R2 class)",
                "ef_class": "PI",
                "residual": "RO8-R2",
            }
        )
    if actual and not actual.startswith("CK-") and actual:
        # transit before true Kappa — known RO8-R1 class when entering CF
        defects.append(
            {
                "severity": "Minor",
                "id": "PB10-MINOR-LABEL",
                "finding": f"Ops transit / label desync sitting observed as {actual} before/during Kappa entry (RO8-R1 class)",
                "ef_class": "PI",
                "residual": "RO8-R1",
            }
        )
...
def run_persona(persona: dict) -> dict:
    ...
    email = f"pb010.kappa.{slug}.{int(time.time())}@example.com"
    print(f"\n=== persona {slug} provision {email}", flush=True)
    st = create_user(email, f"PB010 {persona['label']}", password)
    ...
    c.html_dir = html_dir
    # Point harness HTML/EVID writes for this persona
    ro.HTML = HTML
    ro.EVID = evid
    ...
    for n in range(40):
        if all(k in seen for k in TARGET):
            break
        print(f"  day {idx} kappa_seen={sorted(seen)}", flush=True)
        try:
            ...
            expect = "CK-D1" if "CK-D1" in ro.SNIPPETS else "CI-D1"
            day_out = complete(c, idx, expect)
            actual = detect(day_out)
            defects = classify_defects(day_out, actual)

            if actual and actual.startswith("CK-"):
                day_out = score_kappa(day_out, actual)
                ...
                if actual not in seen and day_out.get("verdict") == "PASS":
                    ...
                    print(
                        f"  kappa FAIL-ish {actual} score={day_out.get('score_over_9')} finished={day_out.get('finished')}",
                        flush=True,
                    )
            else:
                ...
                if actual:
                    all_defects.extend(
                        d for d in defects if d.get("id") == "PB10-MINOR-LABEL"
                    )
                print(
...
def regression_vs_kappa(persona_results: list[dict]) -> dict:
    """Compare cohort Kappa outputs against RO-008 Campaign Kappa baseline."""
    baseline = json.loads(RO008_BASELINE.read_text()) if RO008_BASELINE.exists() else {}
    expected_days = set(baseline.get("summary", {}).get("kappa_days_observed") or TARGET)
    expected_packages = {
        "CK-D1": "CS1-EP001-PKG-3.1-METHOD-OF-MOMENTS",
        "CK-D2": "CS1-EP001-PKG-3.1-MAXIMUM-LIKELIHOOD",
        "CK-D3": "CS1-EP001-PKG-3.1-EFFICIENCY-BIAS-CONSISTENCY-MSE",
        "CK-D4": "CS1-EP001-PKG-3.1-COMPARISON-MSE",
        "CK-D5": "CS1-EP001-PKG-3.1-ASYMPTOTIC-MLE",
        "CK-D6": "CS1-EP001-PKG-3.1-BOOTSTRAP-ESTIMATOR",
        "CK-R1": "CS1-EP001-PKG-REV-ESTIMATORS",
    }
    known_residuals = {"RO8-R1", "RO8-R2", "RO8-R3"}
    regressions = []
    package_mismatches = []
    new_defects = []
    ...
    for pr in persona_results:
        ...
        if days != expected_days and not expected_days.issubset(days):
            regressions.append(
                {
                    "persona": pr.get("slug"),
                    "issue": "missing_kappa_days",
        ...
        for d in (pr.get("summary") or {}).get("defects") or []:
            ...
            if d.get("severity") in ("Critical", "Major"):
                ...
                new_defects.append(d)
            elif d.get("severity") in ("Critical", "Major") and not resid:
                new_defects.append(d)

    # Cross-persona recommendation consistency: same day order
    orders = []
    for pr in persona_results:
    ...
    return {
        ...
        "baseline_kappa_days": sorted(expected_days),
...
def aggregate_programme_metrics(persona_results: list[dict]) -> dict:
    ...
    # Strengthen recommendation_consistency with cross-persona order check
    return out
...
def main() -> int:
    ...
    # Persist suite copy into evidence
    suite_src = Path(__file__).resolve()
    (EVID_REPO / "suite" / "run_pb010.py").write_text(suite_src.read_text())

    probe = mod.Client("pb010_probe")
    ...
    regression = regression_vs_kappa(results)
    prog = aggregate_programme_metrics(results)

    # Cross-persona recommendation consistency override
    if regression.get("cross_persona_sequence_consistent"):
        prog["recommendation_consistency"]["result"] = (
            "PASS"
            if prog["recommendation_consistency"]["result"] == "PASS"
            else prog["recommendation_consistency"]["result"]
        )
    else:
