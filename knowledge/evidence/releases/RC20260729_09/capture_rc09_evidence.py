#!/usr/bin/env python3
"""RC-2026.07.29-09 — post-deploy visual evidence screenshots (production).
...
def main() -> int:
    ...
    with sync_playwright() as p:
        ...
        page.on(
            "pageerror",
            lambda err: evidence["errors"].append(
                {"type": "pageerror", "msg": str(err)}
            ),
        )
        ...
        try:
            ...
            for name, path in (
                ...
                ("09_choose_exam", "/study-plan/wizard/1"),
            ):
    ...
    (OUT_DIR / "README.md").write_text(
        "\n".join(
            [
                "# RC-2026.07.29-09 — Visual Evidence",
                "",
